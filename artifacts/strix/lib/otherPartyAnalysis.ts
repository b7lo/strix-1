...
import { mapToVehicleFrame } from "./sensorUtils";
...
export function analyzeOtherParty(params: {
  ...
  const estimatedSpeedRaw = estimateOtherSpeed(
  ...
  // نقرّب هنا (مرة واحدة) لتُستخدم القيمة نفسها في الوصف وفي الحقل المُرجع.
  const estimatedSpeedKmh = Math.round(estimatedSpeedRaw / 5) * 5;
...
function calculateApproachAngle(
  ...
  // نحوّل إلى إطار المركبة أولاً (تسوية جاذبية + إزاحة yaw) تمامًا كما تفعل
  // detectImpactZone، وإلا تناقضت الزاوية مع منطقة الاصطدام عندما يكون الجوال
  // غير محاذٍ لمحاور السيارة (وينعكس ذلك خطأً على رسم الكروكي).
  const vehicle = mapToVehicleFrame(peakFiltered);

  // Vector التسارع المعكوس (مصدر الصدمة) في إطار المركبة
  const sourceX = -vehicle.vX;
  const sourceY = -vehicle.vY;
  ...
  if (gyroscope && gyroscope.yawRate > 30) {
    ...
    if (gyroscope.dominantAxis === "yaw") {
      ...
      angleDeg += vehicle.vX > 0 ? yawAdjust : -yawAdjust;
    }
  }
