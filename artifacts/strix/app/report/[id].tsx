...
import { getReportView } from "@/lib/reportView";
...
export default function ReportScreen() {
  ...
  // مصدر العرض الموحّد: نسبة ونص ومنطقة متّسقة (يحسم تناقض 75٪/0٪ ووصف المنطقة).
  const view = getReportView(displayReport);
      ...
      >
            ...
            <View style={[styles.summaryRight, { alignItems: isRTL ? "flex-end" : "flex-start" }]}>
              <View style={[styles.summaryTitleRow, { flexDirection: rtl.flexDirection }]}>
                ...
                <Text style={[styles.impactLabel, { color: colors.primary, textAlign: rtl.textAlign }]}>
                  {`${t(`zone.${view.effectiveZone}`, { defaultValue: getDirectionAr(t)[view.effectiveDirection] })}`}
                </Text>
              </View>
        ...
        >
          ...
          <View style={styles.diagramWrap}>
            <AccidentDiagram
              zone={view.effectiveZone ?? "unknown"}
              width={300}
              height={280}
        ...
        {/* تقدير المسؤولية - بطاقتا الطرفين (من مصدر العرض الموحّد) */}
        <View style={[styles.partiesRow, { flexDirection: rtl.flexDirection }]}>
          <View style={[styles.partyCard, { backgroundColor: colors.card, borderColor: "#34C75950" }]}>
            ...
            <Text style={[styles.partyCardPercent, { color: "#34C759" }]}>{fmtPct(view.mineFaultPercent)}</Text>
          </View>
          <View style={[styles.partyCard, { backgroundColor: colors.card, borderColor: "#34C75950" }]}>
            <View style={[styles.partyCardHeader, { flexDirection: rtl.flexDirection, backgroundColor: "#34C75918" }]}>
            ...
            <Text style={[styles.partyCardPercent, { color: "#34C759" }]}>{fmtPct(view.otherFaultPercent)}</Text>
          </View>
        </View>
        ...
        >
          ...
          {view.plainSummary ? (
            <View
              style={[
                styles.summaryBox,
                { backgroundColor: colors.primary + "14", borderColor: colors.primary + "40", flexDirection: rtl.flexDirection },
              ]}
            >
              <Feather name="info" size={16} color={colors.primary} style={{ marginTop: 2 }} />
              <Text style={[styles.summaryText, { color: colors.foreground, textAlign: rtl.textAlign }]}>
                {view.plainSummary}
              </Text>
            </View>
          ) : null}
          ...
          {displayReport.factorsAr && displayReport.factorsAr.length > 0 && (
            <View style={[styles.factorsBox, { backgroundColor: colors.secondary, borderColor: colors.border }]}>
              ...
              {factorsExpanded &&
                (() => {
                  const advHeader = t("liability.advancedAnalysisHeader");
                  return displayReport.factorsAr.map((f, i) => {
                    // سطر "التحليل المتقدم" يُعرض كعنوان فرعي بدل نقطة (تجميع بصري)
                    if (f === advHeader) {
                      return (
                        <Text
                          key={i}
                          style={[styles.factorSubheading, { color: colors.primary, textAlign: rtl.textAlign }]}
                        >
                          {f}
                        </Text>
                      );
                    ...
                    return (
                      <View key={i} style={[styles.factorRow, { flexDirection: rtl.flexDirection }]}>
                        <View style={[styles.factorDot, { backgroundColor: sevColor }]} />
              ...
              {factorsExpanded && (
                <Text style={[styles.termsGlossary, { color: colors.mutedForeground, textAlign: rtl.textAlign }]}>
                  {t("report.termsGlossary")}
                </Text>
              )}
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  summaryText: { flex: 1, fontSize: 14.5, fontWeight: "600", lineHeight: 23 },
  ...
  factorSubheading: { fontSize: 13, fontWeight: "700", marginTop: 10, marginBottom: 2 },
  termsGlossary: { fontSize: 11, lineHeight: 18, marginTop: 12, fontStyle: "italic", opacity: 0.85 },
