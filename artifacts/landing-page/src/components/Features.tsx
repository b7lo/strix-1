...
const features = [
  ...
  },
  {
    ...
    note: 'يعتمد تحديد نسبة المسؤولية على البيانات المسجّلة أثناء الحادث، ويبقى القرار النهائي للجهات المختصة.',
  },
  {
    icon: <FileText size={26} />,
    title: 'تقرير يفهمه الكل',
    desc: 'تقرير فني كامل تشاركه مع الجهات المختصة بضغطة واحدة.',
  },
  {
    icon: <Cloud size={26} />,
    ...
    <section className="features-section" id="features">
      <div className="section-header">
        ...
        <p className="section-sub reveal reveal-delay-2">
          خلف ستركس تقنيات قوية: فلترة إشارات بخوارزميات متقدمة، وتحليل لترددات الاهتزاز لتصنيف الحوادث بدقة عالية.
        </p>
      </div>
      ...
      <div className="features-grid">
        {features.map((f, i) => (
          <div key={i} className={`feature-card reveal reveal-delay-${Math.min(i + 1, 5)}`}>
            ...
            {f.note && <div className="feature-note">{f.note}</div>}
          </div>
        ))}
      </div>
    </section>
