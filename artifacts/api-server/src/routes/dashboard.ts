...
import { desc, eq, sql, and, or, type SQL } from "drizzle-orm";
...
export async function computeDashboardStats(database: Database = db) {
  ...
  // عدد الحوادث المشتركة = عدد أزواج التحقّق المتقاطع (صف واحد لكل زوج في
  // cross_verified_analyses). نقرأ من نفس الجدول الذي تعرض منه صفحة "الحوادث
  // المشتركة" لضمان تطابق البطاقة مع الصفحة تمامًا. (سابقًا كانت البطاقة تعدّ
  // صفوف accidents حيث matched_accident_id غير فارغ، فتَحسب كل زوج مرّتين لأن
  // الحقل يُضبط على الطرفين معًا.)
