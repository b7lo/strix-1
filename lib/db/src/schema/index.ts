...
export const accidentsTable = pgTable(
  "accidents",
  {
    ...
    // المستخدم المالك للحادث (auth.users.id). NULLABLE للتوافق مع الحوادث القديمة
    // المرتبطة بالجهاز فقط. القيد المرجعي (FK) يُفرَض في ترحيل SQL لأن auth.users
    // مُدار بواسطة Supabase وغير معرّف في Drizzle.
    userId: uuid("user_id"),
  ...
  (table) => ({
    ...
    userIdx: index("accidents_user_id_idx").on(table.userId),
